...
| Version control | git + GitHub (`melvinmathew9991/customer-support-app`) | 161 commits, 43 merged PRs as of #63; merge commits, branches kept as history, tags `v0.1.0-sprint1` and `v0.1.0-sprint2` (`docs/Git-Workflow.md`); `main` requires a PR and the `test`/`eval-gate` checks |
...
161 commits and 43 merged pull requests as of #63. By pull request:
...
| #63 | `fix/quality-round-2` | Quality round 2 for #5 and #23: a voice-request rule for statements, answers that turn a restriction around replaced by the restriction; all three held-out targets missed (§10.14); merged as a net improvement, #5 and #23 stay open |
...
### 10.2 Live end-to-end re-verification (2026-09-21, merged `main` at #45, `a60d9f8`, `llama3.2:3b`; before the synthetic KB, so answers name Shopify)
...
### 10.3 Eval harness — state at #45 (`docs/eval/Post-Sprint3-FullEval-2026-09-21.md`, 135 conversations, 11 categories, `llama3.2:3b`; every metric matches the Sprint 3 run except callback precision on the full set). Current figures, on 237 entries and the Brightstall KB, are in §10.12-§10.14 and §11
...
| Brightstall KB, before round 1 (2026-09-23) | 51 answered `rag-*`, dev | 7.8% (4/51), all invented places or steps |
| Round 1 held-out (§10.13) | 22 new questions | 4.5% lenient / 9.1% strict; invented steps 0/22 |
| Round 2 held-out (§10.14) | 20 new questions | 10% (2/20), strict grading |

Target ≤5% is not clearly met on any untuned measure. The last row disagrees with the earlier ones (2% vs about 10%): it includes entries the fixes were tuned on, and the grade is one reader's judgment, so the model *comparison* in §10.5 is the safer reading than that absolute rate. The earlier figures were on the old KB and were not re-graded; the last three rows are on the Brightstall KB, graded by the same single reader who wrote the questions and the fixes.
...
Limits: single observations on one machine, one recording, one local model. Not tried: `llama3.1:8b`, or Linux/macOS (the pin builds on Windows only as far as tested). The golden set does not exercise this path, so it was not re-run. The tool always transcribes the same fixed recording, so every ticket describes that call whoever asked. Without the extra the callback path still works as in §10.2. On 2026-09-23 the recording was replaced by a synthesized Brightstall call (§10.12) and the tool re-run once in a fresh venv: an accurate Whisper transcript in about 43 s and a ticket naming Ruby and Michael, with one paraphrase slip in the summary (who signed out).
...
| Issues | #14 closed (§10.12); #17 closed (§10.13); #5 and #23 accepted as known limits of `llama3.2:3b` after two rounds, open in the Known limits milestone (§10.14) |
| Commits / PRs | 161 commits, 43 merged PRs as of #63; single contributor |
...
| 9 | Hallucination on adversarial tier-crossing questions: retrieval stayed scoped but the answer contradicted its own context | Medium | Partly fixed, open (#5): the round-2 held-out rate is 10% (§10.14); see finding 19 |
...
| 13 | Hallucination rate 3/15 = 20% against ≤5% (Sprint 1 grade) | Medium | Superseded by §10.4 and §10.14; target not met |
...
| 16 | Sprint 1's callback precision of 100% was uninformative: the original negatives had no digits, so the pre-check rejected them before the model ran. Held-out negatives with numbers gave 85%, then 67% on the later cohort | Medium | Measurement fixed; the underlying false triggers are still open (#23): round-2 held-out precision 91%, recall 77% (§10.14) |
...
| 19 | Model invents UI steps for "how do I..." questions the KB doesn't answer | Medium | Fixed (#17, PR #61): plain-prose steps and unsupported places are blocked; 0 of 22 held-out answers invented steps (§10.13) |
...
The work of 2026-09-23 had three practical obstacles and one methodological one. No Chrome extension was available, so the browser check used Playwright driving headless Edge; the first install failed silently on Windows' path-length limit, and the first screenshots captured a page still loading until the check waited for the rendered messages. The flaky save-order test, previously written off as a timing flake, began failing on every run and blocked the merge of #59 into the knowledge-base branch through the pre-commit hook; rather than bypass the hook, the root cause (a 0.5-1 ms Windows clock tie) was found and fixed on its own branch (#62). Retargeting #61 to `main` did not start CI, because the workflow does not trigger on a base change; closing and reopening the PR did. The methodological obstacle was the moving target: each unseen cohort surfaced failure types the previous fixes had not seen, and a dev replay showed the model's answers vary between runs, so the method was tightened (criteria in the same commit as the cohort, strict grading fixed in advance, two verification runs) rather than the fixes tuned further. One limit could not be removed: the same author wrote the cohorts, the fixes and the grades.

## 15. Limitations

**Still true:** on the shipped `llama3.2:3b` default, the round-2 held-out cohort misses callback recall (77%), callback precision (91%) and hallucination (10%) (§10.14) - `gpt-4o-mini` clears the precision target on the same golden set (§10.10), so this is now a limit of the local-model choice, not a limit shown to be inherent to the pipeline; the with-whisper path (real call transcription and ticket summary) works but was exercised only live, three times, on two recordings (the old one twice, the synthesized Brightstall call once): a single fixed sample is always what is transcribed, so every ticket describes that call, and ticket quality on other calls is unknown; identification is still a lookup, not authentication - a real store now backs it (Sprint 4), but anyone who knows an email or phone number is still served that customer's tier, a decision made deliberately, not just deferred (`docs/User-Store-Design.md`).

**No longer true:** no CI; ruff configured but unenforced (27 findings); the free-tier KB contradicting itself (twice); identification bypassable with fabricated input; no way to reindex the KB deterministically; no held-out data; callback extraction returning the wrong number; a model that skips a tool call getting a made-up tier; phone numbers and mixed-case emails not identifying anyone; no cap or timeout on LLM calls (#30); the README steering users to the rejected 8B model; a conversation being lost on every restart or page reload; a failed identification being followed by free-tier answers (#37); a callback request beside a decline being refused whole (#33); identification backed by an in-memory mock instead of a real, persistent store (Phase 7, Sprint 4); the eval harness's own regression checking depending on a human remembering to run it before merge (Sprint 5's `eval-gate`); OpenAI never having been tried at all (Sprint 6 era, §10.10); `app.py` being a bare, unstyled default Streamlit app (Phase 9, Sprint 6); no accounting for how many tokens a turn costs on either provider; third-party sample data of unconfirmed origin in the repository (#14, replaced 2026-09-23); a resumed conversation losing its ticket styling and no real-browser check of the theme (#59); invented steps in answers (#17); a flaky save-order test that hid a real wrong-session bug (#62).
...
**Quick:** #14 and #17 are done (§10.12, §10.13). #5 and #23 were accepted by the maintainer as known limits of `llama3.2:3b` on 2026-09-23, after two rounds (§10.14); they stay open in the Known limits milestone with their current figures in the titles, to be revisited only with a model change. The one concrete gap finding 34 (§13) left open - `eval-gate`/`test` as required status checks - was closed the same day, not left for later (`docs/Git-Workflow.md`).
