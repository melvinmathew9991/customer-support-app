...
| Pipeline | `pipeline.py` | 266 | `CustomerSupportPipeline` orchestration + per-turn structured logging; a model timeout becomes a reply (#30); with a `SessionStore` it saves after every completed turn and can resume a saved conversation by rebuilding the graph (no replay, no model call); `current_user_profile` property backs the Sprint 6 tier badge |
...
| Interfaces | `app.py` | 156 | Streamlit Chat + Graph tabs; the session id lives in the URL (`?session=<id>`), so a reload or server restart resumes; an ended conversation shows its transcript and a start-again button. Themed since Sprint 6: a subscription-tier badge, `st.warning`/`st.success` for retry/ticket-confirmation messages (§10.11) |
...
| Tests | `tests/**/test_*.py` | 3,053 | 371 tests across 20 files, none needing a live model: agents (`test_call_customer_edge` 47, `test_user_info_edge` 9, `test_call_customer_node` 4, `test_authenticated_user_node` 2), domain (`test_chat` 4), graph (`test_retrieval_guard` 14, `test_node` 5, `test_text_based_edge` 3, `test_edge` 2), tools (`test_user_store` 35, `test_rag_responder` 8 - `test_user_info_db.py` was retired in Sprint 4 with the mock DB it tested), eval (`test_golden_set` 124, `test_run_eval_scoring` 19, `test_eval_gate` 10), and top level (`test_session_store` 22, `test_pipeline_persistence` 24, `test_cli` 14, `test_config` 12, `test_app_sessions` 12, `test_pipeline_timeout` 1) |
...
├── tests/                 # 371 unit tests (deterministic logic only)
...
| Testing | pytest 8.3.3 | 371/371 pass (20 files, up from 335/19 - Sprint 6 added 7 tests to `test_app_sessions.py`/`test_pipeline_persistence.py`) |
...
- **Not done:** a real-browser visual check of the theming in either mode - no Chrome browser-automation tool was connected in the session that built this. See §10.11, §13 finding 36.
...
| Metric | Value |
|---|---|
| Unit tests passing | 371/371 (20 files) |
...
| Issues | #14 closed by the synthetic KB (§10.12); #5, #17, #23 open |
...
- A resumed session's historical ticket-confirmation messages replay as plain text; only live, same-process ones get styled, since which node produced a past message isn't persisted.
- No real-browser visual check was done in either theme (§13 finding 37) - the one open item before `Design.md` §5's usability pass is fully satisfied, not just structurally satisfied.
...
**Quick:** #14 is done (§10.12). #5, #17 and #23 stay open as accepted known limits (`docs/Sprints.md`). The one concrete gap finding 34 (§13) left open - `eval-gate`/`test` as required status checks - was closed the same day, not left for later (`docs/Git-Workflow.md`).
...
**Immediate:** a real-browser visual check of Sprint 6's theming in both light and dark mode (§13 finding 37) - the one item left before `Design.md` §5 is fully, not just structurally, satisfied.
...
- The product (Phases 1-5) already worked before this engagement; it is now *provably* measured and every sprint on the roadmap through Sprint 6 is done — a 135-conversation golden set, 371 unit tests, a CI regression gate that is now a *required* status check, conversations that survive a restart, a real persistent user store, a themed UI, and held-out entries committed before each run — and was re-exercised live on the merged `main` for this update (7 of 7 checks).
...
- Two accuracy targets are still missed with the shipped local model, `llama3.2:3b`: hallucination (about 13% on untuned questions vs ≤5%) and callback precision on unseen messages (83%/94% vs ≥95%). A larger local model was tried and did not help; **a real, token-measured comparison against OpenAI's `gpt-4o-mini` did** - it cleared the precision target on the identical golden set for about three cents - narrowing "known limit" to a cost of staying local rather than a ceiling on the pipeline itself. A process evaluation found the method strong and the statistics weak and proposed a plan that is not yet adopted; the only sprint left on the roadmap is deployment and observability (Sprint 7).

### One-line description

A graph-orchestrated, fully-local-by-default LLM customer support agent whose original build was sound but unmeasured — this engagement built the evaluation infrastructure that measured it, fixed the real bugs and one live-traffic incident it exposed, made conversations survive a restart, gave it a real user store and a themed UI, measured a real alternative backend against it, and reports honestly which accuracy targets the shipped small local model still does not reach, which of those a paid one does, and how far the evidence can be trusted.
